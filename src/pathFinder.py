...
class PathFinder:
    ...
    def plot_curve(self, ax, init_point, init_orient, velocity, path_nodes, bot_path_cost, color, bot_status, cmap_plotter, c_space, pause=False):
        ...
        while t < velocity[TRANS][TIME_IND]:
            ...
            ax.plot([prev_x, new_x], [prev_y, new_y], color=color, linewidth=3)
            if c is not None:
                c.remove()
            ...
            pts = self.transform_pts(pts,theta=orient)
            pts[:,0]+=new_x
            pts[:,1]+=new_y
        ...
        # pts = np.vstack((pts,pts[0]))
        pts = self.transform_pts(pts,theta=1)
        cir = plt.Polygon(pts, fc='red', ec='black', closed=True)
        cir = plt.Circle((prev_x, prev_y), 0.2, fc=None)
        # plt.gca().add_patch(cir)
        ax.add_patch(cir)

        return new_x, new_y, math.degrees(orient), bot_path_cost
    
    def transform_pts(self,pts,theta=1):
        pts= ( np.dot(np.array([ [math.cos(theta), -math.sin(theta)], [math.sin(theta), math.cos(theta)]]) , pts.T) ).T
        return pts
